...
public class UsuarioController {
	...
	public String intranet(Authentication auth, Model model, HttpSession session) {
		...
		if (u.getRol().getIdRol() == 1) {
			Menu menuPrestamista = lista.stream().filter(item -> "Prestamista".equals(item.getDescripcion()))
	...
	public String mostrarFormularioRegistroUsuario(Model model, HttpSession session, @PathVariable int rolId,
	...
	public String editarPrestamista(@PathVariable(name = "id") int id, Model model, HttpSession session,
		...
		if (id > 0) {
			...
			// Retorna un redirect a la URL /listarUsuario para mostrar la lista con el
			// atributo error que almacena
			// el mensaje
			return "redirect:/listarUsuario";
		}
	...
	public String eliminarPrestamista(@PathVariable(name = "id") int id,
		...
		// Valida que el id sea mayor a 0
		if (id > 0) {
			// Si es válido, llama al método eliminarPrestamista del servicio, pasándole el
			// id
